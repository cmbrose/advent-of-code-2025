...
func main() {
	...
	// Find the longest 3 circuits
	lengths := util.Map(util.Keys(circuits), func(circuit *Circuit) int {
		return len(circuit.breakers)
	})
	...
	ans := lengths[len(lengths)-1] * lengths[len(lengths)-2] * lengths[len(lengths)-3]
